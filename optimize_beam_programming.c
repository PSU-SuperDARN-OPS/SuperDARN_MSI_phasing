...
/* Initialize the opt arrays with obviously bogus data */
    ...
    if (freq_steps > MSI_max_freq_steps) {
        fprintf(stderr,
                "Number of frequency steps too large for MSI phase card memory to support! Check your MSI frequency windowing settings: min/max_freq and freq_window\n");
        exit(-1);
    }
    ...
    mypause();
    /****************************************************
     * Initialize DIO cards
    ***************************************************/
    phasing_matrix.radar_number = rnum;
    temp = set_ports(&phasing_matrix);
    if (temp < 0) {
        printf("Invalid radar number %d \n", phasing_matrix.radar_number);
        printf("Breaking...\n");
        exit(-1);
    }
...
//                 exit(-1);
        ...
        for (a = 0; a < MSI_num_angles; a++) {
            ...
            if (verbose > -1)
                fprintf(stdout, "    Angle Index: %5d : %lf [deg] :: %8.4lf [nsec]\n", a, angles_degrees[a],
                        timedelay_nsecs);
...
//                    exit(0);
                ...
                if (nave >= VNA_min_nave) {
                    ...
                    pwr_ave = -1E13;
                    fprintf(stdout,
                            "        Warning:: low nave: %d < desired: %d >. Consider reconfiguring VNA or freq windows\n",
                            nave, VNA_min_nave);
                }
                if (verbose > 1) {
                ...
                while (fast_loop < 3) {
                    ...
                    if (tdelta <= MSI_tdelay_tolerance_nsec) {
                        ...
                        while (wflag == 1) {
                            ...
                            if (tdelta < test_tdelta) {
                                ...
                                fprintf(stderr,
                                        " Info:: New best Tdelay! %d :: best: %13.4lf code: %8d delta: %13.4E\n", count,
                                        best_tdelay, best_phasecode, test_tdelta);
                            } else {
                                fprintf(stderr,
                                        " Info:: Tdelay error got worse! %d :: best: %13.4lf current: %13.4lf\n", count,
                                        test_tdelta, tdelta);
                            }
                            if (count > 8) {
                                fprintf(stderr, " Info:: Count limit reached %d. Move on to brute force. \n", count);
                    ...
                    fprintf(stdout,
                            "          Fast Optimized pcode: %5d :: tdelay [ns]:: Measured: %13.4lf Needed: %13.4lf tdelta: %13.4lf\n",
                            best_phasecode, best_tdelay, needed_tdelay, tdelta);
                    fprintf(stdout,
                            "          Fast Optimized acode: %5d :: Gain   [dB]:: Measured: %13.4lf Needed: %13.4lf adelta: %13.4lf\n",
                ...
                time_spent_step =
                        (end_step.tv_sec - begin_step.tv_sec) + 1E-9 * (end_step.tv_nsec - begin_step.tv_nsec);
                time_spent_card =
                        (end_step.tv_sec - begin_card.tv_sec) + 1E-9 * (end_step.tv_nsec - begin_card.tv_nsec);
                ...
                fprintf(stdout, "      -------\n");
                fprintf(stdout,
                        "      Optimize End:: MemLoc: %5d Q: %5d Angle: %5d Freq Step: %5d : %-8.5e - %-8.5e [Hz]\n", b,
                        opt_qual[b], a, f, opt_freq_lo[b], opt_freq_hi[b]);
            ...
            time_spent_angle =
                    (end_angle.tv_sec - begin_angle.tv_sec) + 1E-9 * (end_angle.tv_nsec - begin_angle.tv_nsec);
        ...
        if (b >= MSI_phasecodes) {
            ...
            if (opt_qual[b] != -1) {
                ...
                fprintf(stderr,
                        "           Card: %5d MemLoc: %5d Q: %5d Angle: %5d Freq Step: %5d : %-8.5e - %-8.5e [Hz]\n", c,
                        b, opt_qual[b], a, f, opt_freq_lo[b], opt_freq_hi[b]);
                fprintf(stderr, "           tdelay [ns]:: Min: %13.4lf Max: %13.4lf Ave: %13.4lf\n", opt_tdelay_min[b],
                        opt_tdelay_max[b], opt_tdelay_ave[b]);
                fprintf(stderr, "           gain   [dB]:: Min: %13.4lf Max: %13.4lf Ave: %13.4lf\n", opt_gain_min[b],
                        opt_gain_max[b], opt_gain_ave[b]);
            }
            opt_pcode[b] = 8191;
            opt_acode[b] = 63;
            opt_qual[b] = 0;
        ...
        for (b = 0; b < MSI_phasecodes; b++) {
            if (opt_qual[b] >= 0) {
                if (verbose > -1) {
                    fprintf(stdout,
                            "           Card: %5d MemLoc: %5d Q: %5d Bmnum: %5d Angle: %13.4lf [deg] Freq Range: %-8.5e - %-8.5e [Hz]\n",
                            c, b, opt_qual[b],
                            opt_bmnum[b], opt_bmangle_deg[b],
                            opt_freq_lo[b], opt_freq_hi[b]
                    );
                    fprintf(stdout,
                            "             pcode: %5d :: tdelay [ns]:: Min: %-8.5e Max: %-8.5e Ave: %-8.5e Target: %-8.5e Delta: %-8.5e\n",
                            opt_pcode[b], opt_tdelay_min[b], opt_tdelay_max[b], opt_tdelay_ave[b], opt_tdelay_target[b],
                            fabs(opt_tdelay_ave[b] - opt_tdelay_target[b]));
                    fprintf(stdout,
                            "             acode: %5d :: gain   [dB]:: Min: %-8.5e Max: %-8.5e Ave: %-8.5e Target: %-8.5e Delta %-8.5e\n",
